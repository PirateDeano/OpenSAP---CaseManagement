...
sap.ui.controller("sap.ui.demo.myFiori.view.Master", {
	...
	handleSearch : function (evt) {
		...
		if (query && query.length > 0) {
			var filter = new sap.ui.model.Filter("CaseId", sap.ui.model.FilterOperator.Contains, query);
			filters.push(filter);
		}
	...
	handleGroup : function (evt) {
		...
		if ("CompanyName" === key || "LifecycleStatus" === key) {
...
this.getView().getModel("i18n").getResourceBundle();
	...
	handleCalendarView: function (evt){
		var context = evt.getSource().getBindingContext();
		this.nav.to("Calendar", context);
	}
